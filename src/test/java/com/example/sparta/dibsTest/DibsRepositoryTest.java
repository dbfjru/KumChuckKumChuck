...
public class DibsRepositoryTest {
  ...
  @Autowired
  DibsQueryRepository dibsQueryRepository;
  ...
  void test1() {
    ...
    Dibs removedibs = dibsQueryRepository.findDibsByStoreAndUser(store, user);
    if (removedibs == null) {
      throw new NoSuchElementException("해당 가계는 찾을수 없어요");
    }
